...
from afloclient.common.utils import safe_header
...
class HTTPClient(object):
    ...
    def __init__(self, endpoint, **kwargs):
        ...
        if self.endpoint.startswith("https"):
            ...
            if not compression:
                self.session.mount("aflo+https://", https.HTTPSAdapter())
                self.endpoint = 'aflo+' + self.endpoint
    ...
    def _request(self, method, url, **kwargs):
        ...
        # Read body into string if it isn't obviously ticket data
        if content_type == 'application/octet-stream':
            # Do not read all response in memory when
            # downloading an ticket.
