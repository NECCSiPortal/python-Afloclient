...
class HTTPSAdapter(adapters.HTTPAdapter):
    ...
    https pool by setting afloclient's
    one.
    """
    ...
    def _create_aflo_httpsconnectionpool(self, url):
    ...
    def get_connection(self, url, proxies=None):
        try:
            ...
            # The scheme is most likely aflo+https but check anyway
            if not url.startswith('aflo+https://'):
                raise

            return self._create_aflo_httpsconnectionpool(url)
...
class HTTPSConnectionPool(connectionpool.HTTPSConnectionPool):
    ...
    returns an instances of afloclient's VerifiedHTTPSConnection
...
class VerifiedHTTPSConnection(HTTPSConnection):
    ...
    def __init__(self, host, port=None, key_file=None, cert_file=None,
        ...
        if six.PY3:
            excp_lst = (TypeError, exc.FileNotFoundError, ssl.SSLError)
        else:
            # NOTE(jamespage)
